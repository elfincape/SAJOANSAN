...
context.COL_MAP = Object.fromEntries(['primary_driver_name', 'primary_driver_phone', 'driver_health', 'stop_order'].map(key => [key, { key }]));
context.ALL_KEYS = Object.keys(context.COL_MAP);
context.state = { colOrder: ['primary_driver_name', 'stop_order'] };
assert.equal(context.orderedColumns().map(c => c.key).join(','), 'primary_driver_name,primary_driver_phone,driver_health,stop_order');
context.state.colOrder = ['primary_driver_phone', 'stop_order', 'primary_driver_name'];
assert.equal(context.orderedColumns().map(c => c.key).join(','), 'primary_driver_phone,driver_health,stop_order,primary_driver_name');
