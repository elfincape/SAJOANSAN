...
await assert.rejects(prepareCompilation({list:async()=>records.slice(1)},'driver'),/8장/);
...
let role='editor',count=8,stale=false,configured=true,prior=null,existing=null,puts=0,graphFail=false,dbFail=false;
...
const fetcher=async(input,options={})=>{
 ...
 if(p.startsWith('/rest/v1/')){
  ...
  if(table==='driver_documents')return response(records.slice(0,count).map(r=>({...r,request_id:stale?'changed':r.request_id})));
...
const handler=makeHandler(env,fetcher,async images=>{
 assert.equal(images.length,8);images.forEach((b,i)=>assert.equal(b[3],i));return pdf;
});
const request=()=>{
