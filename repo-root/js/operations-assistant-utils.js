...
export function normalizeAssistantResponse(value){
  ...
  return {answer:String(value.answer||''),columns:Array.isArray(value.columns)?value.columns.slice(0,30).map(c=>({key:String(c.key||''),label:String(c.label||c.key||''),type:c.type==='number'?'number':'text'})).filter(c=>c.key):[],rows:Array.isArray(value.rows)?value.rows:[],notices:Array.isArray(value.notices)?value.notices.map(String).slice(0,10):[],meta:value.meta&&typeof value.meta==='object'?value.meta:{}};
}
