...
export function parseCharterSheet(sheet, XLSX) {
  ...
  function readCell(key) {
    ...
    const errorText = /^#(?:N\/A|VALUE!|REF!|DIV\/0!|NAME\?|NUM!|NULL!|SPILL!|CALC!|GETTING_DATA|FIELD!|BLOCKED!|UNKNOWN!|CONNECT!|BUSY!|PYTHON!)$/i;
    const failed = cell?.t === 'e' || errorText.test(String(cell?.v ?? '').trim()) || (cell?.f && cell.v == null);
    let value = cell;
    if (failed) {
      const code = cell.w || ({0:'#NULL!',7:'#DIV/0!',15:'#VALUE!',23:'#REF!',29:'#NAME?',36:'#NUM!',42:'#N/A',43:'#GETTING_DATA'}[cell.v]) || String(cell.v ?? '수식 결과 없음');
      warnings.push(`${key}: ${code} 오류로 결과를 빈칸 처리했습니다. 확인이 필요합니다.`);
      value = { t: 's', v: '', error: true };
    }
  ...
  for (let r = 3; r <= range.e.r; r++) {
    ...
    const date = readCell(XLSX.utils.encode_cell({ r: start, c: merge?.s.c ?? 1 })), destination = readCell(`G${r + 1}`);
    if (blank(date) && !date?.error) {
    ...
    for (const [col, field] of [[16, 'sourceQ'], [18, 'sourceS']]) {
      ...
      const cell = readCell(XLSX.utils.encode_cell({ r: m?.s.r ?? r, c: m?.s.c ?? col }));
      if (cell?.error) { group[field] = cell; continue; }
      if (group[field]?.error) continue;
    ...
    const address = readCell(addressKey);
    const stopKey = blank(address) && !address?.error ? `missing:${r}` : addressKey;
    if (!group.stops.has(stopKey)) group.stops.set(stopKey, { destinations: [], city: '', rate: null, hasAddress: !blank(address) || !!address?.error });
    ...
    if (!blank(destination) || destination?.error) {
