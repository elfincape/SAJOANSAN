...
async function restAll(e:ReturnType<typeof envs>,token:string,table:string,query:string){const clean=query.replace(/(?:^|&)limit=\d+/g,'').replace(/^&/,'');const rows:any[]=[];for(let from=0;;from+=REST_PAGE_SIZE){const response=await fetch(`${e.url}/rest/v1/${table}?${clean}`,{headers:{apikey:e.anon,authorization:`Bearer ${token}`,accept:'application/json',Range:`${from}-${from+REST_PAGE_SIZE-1}`},signal:AbortSignal.timeout(20000)});if(!response.ok)throw coded('QUERY_FAILED',502);const page=await response.json();rows.push(...page);if(page.length<REST_PAGE_SIZE)break}return rows}
async function safeRestVariants(e:ReturnType<typeof envs>,token:string,table:string,queries:string[]){let last:unknown=null;for(const query of queries){try{return await restAll(e,token,table,query)}catch(error){last=error}}console.warn(JSON.stringify({table,code:(last as any)?.code||'QUERY_FAILED',fallback:'empty'}));return[]}
...
async function buildOperationsSnapshot(e:ReturnType<typeof envs>,token:string,center:string):Promise<Snapshot>{
  const companies=await safeRestVariants(e,token,'companies',[`select=id,name&center_code=eq.${center}`,`select=id,name`]);
  const routes=await safeRestVariants(e,token,'routes',[
    `select=id,name,car_number,company_id,primary_driver_id,secondary_driver_id,primary_vehicle_id,secondary_vehicle_id,active,closed_days&center_code=eq.${center}`,
    `select=id,name,car_number,company_id,primary_driver_id,secondary_driver_id,primary_vehicle_id,secondary_vehicle_id,active&center_code=eq.${center}`,
    `select=id,name,car_number,company_id,primary_driver_id,primary_vehicle_id,active&center_code=eq.${center}`
  ]);
  const [stops,points,drivers,vehicles,courseRowsRaw]=await Promise.all([
    ...
    safeRestVariants(e,token,'delivery_points',[
      `select=id,code,name,region,address,contact,delivery_method,access_method,delivery_location,deadline_business_min,allow_under_1ton,allow_under_3_5ton,allow_over_5ton,allow_unmanned_yard,security_key_location,security_password&center_code=eq.${center}`,
      `select=id,code,name,region,address,contact,delivery_method,access_method,delivery_location,deadline_business_min,allow_under_1ton,allow_under_3_5ton,allow_over_5ton,allow_unmanned_yard&center_code=eq.${center}`,
      `select=id,code,name,region,address,contact,delivery_method,access_method,delivery_location,deadline_business_min&center_code=eq.${center}`
    ]),
    safeRestVariants(e,token,'drivers',[`select=id,name,phone,company_id&center_code=eq.${center}`,`select=id,name,phone&center_code=eq.${center}`]),
    safeRestVariants(e,token,'vehicles',[`select=id,plate_number,tonnage,pallet_count,company_id&center_code=eq.${center}`,`select=id,plate_number,tonnage,company_id&center_code=eq.${center}`,`select=id,plate_number&center_code=eq.${center}`]),
...
스냅샷:${JSON.stringify(snapshot)}`;
  const response=await fetch(AI_URL,{method:'POST',headers:{'content-type':'application/json','x-api-key':key,'anthropic-version':'2023-06-01'},body:JSON.stringify({model:MODEL,max_tokens:64000,messages:[{role:'user',content:prompt}]}),signal:AbortSignal.timeout(120000)});
