...
export function validatePlan(value:unknown):Plan{
  ...
  const strings=['vehicleQuery','driverQuery','companyQuery','deliveryPointQuery','region','routeQuery','missingField','vehicleClass'];for(const k of strings)if(typeof raw[k]==='string')args[k]=String(raw[k]).trim().slice(0,100);
