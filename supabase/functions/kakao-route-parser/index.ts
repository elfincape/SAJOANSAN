import {mergeRouteRows,parseKakaoBaseline,type RouteRow} from './parser.ts';
...
Deno.serve(async req=>{
  ...
  try{
    ...
    const body=await req.json().catch(()=>null),text=String(body?.text||'').trim();
    if(!text||text.length>MAX_TEXT_LENGTH)return fail('텍스트 파일 내용을 확인해주세요.',400);
    const baseline=parseKakaoBaseline(text),rows=await organize(text,env.aiKey,baseline);
    return json({rows});
...
async function organize(text:string,key:string,baseline:RouteRow[]){
  const prompt=`카카오톡 대화 내보내기 텍스트를 운송 팔레트 표로 정리한다.
...
안산-김해와 부산-안산으로 시작하는 메시지만 사용한다.
안산-김해와 부산-안산은 완전히 별개로 계산한다. 양쪽 기사, 전화번호, 차량번호가 같을 필요가 없다.
각 날짜와 각 방향에서 대화에 먼저 등장한 운행을 1호차, 다음 운행을 2호차로 정한다. 이름, 전화번호, 차량번호는 결과에 넣지 않는다.
...
형식: {"rows":[{"date":"2026-07-16","weekday":"목요일","ansanGimhae1":10,"ansanGimhae2":10,"busanAnsan1":12,"busanAnsan2":10}]}
서버가 먼저 추출한 참고값은 비어 있는 결과를 보완하는 용도이며 알려진 부산-안산 p 값을 삭제하지 않는다.
...
입력 텍스트:\n${text}`;
  ...
  return mergeRouteRows(normalizeRows(parsed?.rows),baseline);
}
...
function normalizeRows(value:unknown):RouteRow[]{if(!Array.isArray(value))return[];return value.slice(0,1000).map((row:any)=>({date:normalizeDate(row?.date),weekday:String(row?.weekday||'').trim(),ansanGimhae1:pallet(row?.ansanGimhae1),ansanGimhae2:pallet(row?.ansanGimhae2),busanAnsan1:pallet(row?.busanAnsan1),busanAnsan2:pallet(row?.busanAnsan2)})).filter(row=>row.date).sort((a,b)=>a.date.localeCompare(b.date));}
