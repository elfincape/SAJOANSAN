...
Deno.serve(async req=>{
  ...
  try{
    ...
    const rows=await organize(text,env.aiKey);
    return json({rows});
...
async function organize(text:string,key:string){
...
각 날짜와 방향별로 서로 다른 차량번호의 최초 등장 순서에 따라 1호차, 2호차를 정한다. 같은 차량의 후속 메시지는 같은 호차다.
각 호차 메시지 전체에서 마지막으로 나온 숫자+p 값을 팔레트 수로 사용한다. p 값이 없으면 빈 문자열이다.
...
function normalizeRows(value:unknown){if(!Array.isArray(value))return[];return value.slice(0,1000).map((row:any)=>({date:normalizeDate(row?.date),weekday:String(row?.weekday||'').trim(),ansanGimhae1:pallet(row?.ansanGimhae1),ansanGimhae2:pallet(row?.ansanGimhae2),busanAnsan1:pallet(row?.busanAnsan1),busanAnsan2:pallet(row?.busanAnsan2)})).filter(row=>row.date).sort((a,b)=>a.date.localeCompare(b.date));}
